...
if (typeof window.logDebug === 'undefined') {
    ...
    window.toggleDebugMode = function() {
        alert("Módulo debug.js no cargado. Añade <script src='debug.js'></script> al HTML.");
    };
...
function renderTarjetaR34(item) {
    ...
    // CAMBIO: Pasamos 'currentBooru' en lugar de 'r34' fijo
    // Esto le dice a renderCard de dónde viene la imagen
    renderCard(src, prev, type, item.tags, '', currentBooru);
}

// --- REDDIT ---
function buscarReddit() {
    ejecutarBusqueda();
}
...
async function cargarCatalogo4Chan() {
    ...
    try {
        ...
        catalogCache = [];
        pages.forEach(p => {
            if (p.threads) catalogCache.push(...p.threads);
        });
...
async function fetchNitter(query) {
    ...
    return {
        ok: false,
        error: "Todos los mirrors de Nitter fallaron."
    };
}
async function cargarX() {
    ...
    try {
        ...
        if (xml.querySelector("parsererror")) {
            document.getElementById('loading-status').innerText = "Error XML.";
            return;
        }
        document.getElementById('loading-status').style.display = 'none';
        const items = xml.querySelectorAll('item');
        if (items.length === 0) {
            document.getElementById('loading-status').innerText = "Sin resultados.";
            return;
        }
        items.forEach(item => {
            ...
            if (img) {
                renderCard(img[1], img[1], 'img', item.querySelector('title').textContent, 'X', 'x');
            }
        });
    } catch (e) {
...
function renderCard(src, prev, type, tags, badgeTxt, context) {
    ...
    if (context === 'r34' || context === 'booru_generic' || context === 'reddit') {
        ...
        if (context === 'reddit') {
            ...
            // Generamos el texto del badge basado en el modo actual
            // Si es 'anime_pictures', mostramos 'AP', si es 'r34' mostramos 'R34', etc.
            let siteLabel = context === 'r34' ? 'R34' : context.substring(0, 4).toUpperCase();
            if (context === 'anime_pictures') siteLabel = 'AP';

            // Color dinámico: Verde para safe, Azul para R34, Rosa para Anime
            let badgeColor = '#3b82f6';
            if (context === 'anime_pictures') badgeColor = '#ff77aa';
            if (context === 'safebooru') badgeColor = '#22c55e';
...
// --- OBSERVERS INTELIGENTES (AUTOPLAY) ---

// Configuración: El video debe estar al 70% visible para arrancar
...
const videoObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
            ...
            // Si el video no tiene fuente cargada (preload=none), forzamos carga
            ...
            if (playPromise !== undefined) {
                playPromise.catch(error => {
                    // Si falla (navegador bloquea audio automático),
                    // silenciamos e intentamos de nuevo.
                    console.log("Autoplay bloqueado, intentando muteado...");
                    video.muted = true;
                    video.play();
                });
            }
            // Añadimos clase para efectos visuales si quisieras
            video.closest('.media-wrapper').classList.add('playing');

        } else {
            // --- VIDEO FUERA DE FOCO: PAUSE ---
            video.pause();
            // Opcional: Reiniciar al principio para que siempre empiece de cero al volver
            // video.currentTime = 0; 
...
// Observer para carga infinita (Se mantiene igual)
const scrollObserver = new IntersectionObserver((es) => {
    if (es[0].isIntersecting && !cargando && hayMas && (modoActual === 'r34' || modoActual === 'reddit')) {
        cargarSiguientePagina();
    }
}, {
    rootMargin: '600px'
}); // Aumentado margen a 600px para que cargue antes de llegar al final
...
inp.addEventListener('input', (e) => {
    // --- FRENO DE SEGURIDAD ---
    // Si no estamos en Rule34, no usamos su autocompletado (AP tiene tags distintos)
    if (modoActual !== 'r34' && currentBooru !== 'r34') {
...
window.onload = function() {
    ...
    try {
        if (!SYS_PASS) console.error("Drivers.js no cargado!");
    } catch (e) {}

    // Focus en el input de contraseña si está bloqueado
...
function initSecurityCheck() {
    // Verificamos si ya tiene el "Pase VIP" en memoria
    const token = localStorage.getItem('sys_access_token');

    // Usamos una versión simple del hash de tu contraseña (admin123)
    // Para "admin123", el hash simple que usaremos como ejemplo es 'session_ok_v1'
    // O mejor aún, comparamos contra la variable SYS_PASS de drivers.js

    if (token === 'granted') {
        // Si tiene pase, destruimos el muro
        const wall = document.getElementById('security-wall');
        if (wall) wall.style.display = 'none';
        return true; // Acceso permitido
    } else {
        // Si no, el muro se queda ahí
...
function verifyAccess() {
    ...
    // Compara con la contraseña maestra definida en drivers.js
    if (input.value === SYS_PASS) {
        // ÉXITO: Guardamos el pase en memoria
        localStorage.setItem('sys_access_token', 'granted');

        // Ocultamos el muro
        document.getElementById('security-wall').style.display = 'none';

        // Iniciamos la carga automática si había algo pendiente (opcional)
        // Pero como el app.js ya cargó por debajo, ya está listo.
    } else {
        // ERROR
